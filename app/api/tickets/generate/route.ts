...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get company info for commission calculation and MoniMe integration
    const { data: company, error: companyError } = await supabaseAdmin
      .from('companies')
      .select('id, commission_rate, monime_account_id')
    ...
    // Validate MoniMe Account ID (Space ID) is configured
    if (!company.monime_account_id || typeof company.monime_account_id !== 'string' || company.monime_account_id.trim().length === 0) {
      return NextResponse.json(
        { error: 'MoniMe Account ID (Space ID) is not configured for this company. Please configure it in Company Settings.' },
        { status: 400 }
      )
    }
    ...
    // Call MoniMe API to generate multiple dedicated codes
    const monimeClient = createMoniMeClient()
    const normalizedSpaceId = company.monime_account_id.trim()

    const monimeResult = await monimeClient.generateMultipleCodes({
      quantity,
      route_id: route.monime_route_id || route.id,
    ...
    if (!monimeResult.success || !monimeResult.codes || monimeResult.codes.length === 0) {
      return NextResponse.json(
        { error: monimeResult.error || 'Failed to generate codes from MoniMe. Please try again.' },
        { status: 500 }
      )
    }

    // Generate tickets with order numbers from MoniMe
    const tickets = []
    const now = new Date()
    const expiresAt = new Date(now)
    expiresAt.setHours(expiresAt.getHours() + 24) // Valid for 24 hours

    for (const codeData of monimeResult.codes) {
      const orderNumber = codeData.order_number
      ...
      const { data: ticket, error: ticketError } = await supabaseAdmin
        ...
        .insert({
          ...
          status: 'pending',
          expires_at: codeData.expires_at || expiresAt.toISOString(),
      ...
      if (ticketError) {
        // If duplicate order number, log error but continue
        if (ticketError.code === '23505') { // Unique violation
          console.error(`Duplicate order number from MoniMe: ${orderNumber}`)
          continue
        }
    ...
    // If no tickets were created successfully, return error
    if (tickets.length === 0) {
      return NextResponse.json(
        { error: 'Failed to create any tickets. All order numbers may have been duplicates.' },
        { status: 500 }
      )
    }
