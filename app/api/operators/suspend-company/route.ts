...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (userData.role !== 'company_admin') {
      return NextResponse.json(
        { error: 'Only company admins can suspend operators' },
        { status: 403 }
      )
    }
    ...
    // Get operator and verify it belongs to the company admin's company
    ...
    // Verify the operator belongs to the company admin's company
    if (operator.company_id !== userData.company_id) {
      return NextResponse.json(
        { error: 'You can only suspend operators from your own company' },
        { status: 403 }
      )
    }
